...
<body class="min-h-screen flex items-end md:items-center justify-center px-4 py-6">
    <div class="w-full max-w-md bg-[#fffdfa] rounded-[2rem] shadow-2xl p-6 border border-white/70">
        <h1 class="text-2xl font-black text-slate-900">Lupa Kata Sandi</h1>
        <p class="text-sm text-slate-500 mt-2 mb-5">Masukkan email akun. Sistem akan mengirim tautan reset kata sandi.</p>
        ...
        <form method="POST" action="{{ route('password.email') }}" class="space-y-4">
            @csrf
            <input type="email" name="email" required autofocus value="{{ old('email') }}" class="w-full min-h-[52px] rounded-2xl border-slate-200 bg-stone-50 px-4 text-base focus:border-teal-600 focus:ring-teal-600" placeholder="Surel">
            <button class="w-full rounded-2xl bg-teal-600 text-white py-3 font-black">Kirim Tautan Atur Ulang</button>
        </form>

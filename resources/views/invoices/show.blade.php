...
<div class="grid md:grid-cols-4 gap-4 mb-6">
    ...
    <div class="bg-white rounded-xl shadow p-4"><div class="text-xs text-slate-500">Gabus</div><div class="font-bold">{{ $invoice->total_boxes }}</div></div>
...
<div class="grid lg:grid-cols-2 gap-6">
    <div class="bg-white rounded-xl shadow overflow-hidden"><div class="p-4 border-b font-semibold">Pembeli</div><table class="min-w-full text-sm"><thead class="bg-slate-50"><tr><th class="p-3 text-left">Pembeli</th><th class="p-3 text-left">Ikan</th><th class="p-3 text-right">Gabus</th><th class="p-3 text-right">Harga</th><th class="p-3 text-right">Subtotal</th></tr></thead><tbody>@foreach($invoice->items as $item)<tr class="border-t"><td class="p-3">{{ $item->buyer?->name }}</td><td class="p-3">{{ $item->fish_type }}</td><td class="p-3 text-right">{{ $item->box_count }}</td><td class="p-3 text-right">{{ \App\Support\Money::rupiah($item->price_per_box) }}</td><td class="p-3 text-right">{{ \App\Support\Money::rupiah($item->subtotal) }}</td></tr>@endforeach</tbody></table></div>

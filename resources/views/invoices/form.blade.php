...
<div class="border rounded-xl overflow-hidden" id="items-wrapper">
    <div class="bg-slate-50 p-3 flex justify-between items-center"><div class="font-semibold">Detail Pembeli</div><button type="button" onclick="addItemRow()" class="text-sm bg-slate-800 text-white px-3 py-1.5 rounded">Tambah Baris</button></div>
    <div class="overflow-x-auto"><table class="min-w-full text-sm"><thead><tr><th class="p-2 text-left">Pembeli</th><th class="p-2 text-left">Jenis Ikan</th><th class="p-2 text-right">Gabus</th><th class="p-2 text-right">Harga/Gabus</th><th class="p-2 text-right">Subtotal</th><th></th></tr></thead><tbody id="item-rows">
    @php $oldItems = old('items', $invoice?->items?->toArray() ?? [['buyer_id'=>'','fish_type'=>'','box_count'=>'','price_per_box'=>'','notes'=>'']]); @endphp
    @foreach($oldItems as $i => $item)
        <tr class="item-row border-t">
            <td class="p-2"><select name="items[{{ $i }}][buyer_id]" required class="rounded-lg border-slate-300 min-w-48"><option value="">Pilih</option>@foreach($buyers as $buyer)<option value="{{ $buyer->id }}" @selected(($item['buyer_id'] ?? '') == $buyer->id)>{{ $buyer->name }}</option>@endforeach</select></td>
...
<div class="grid md:grid-cols-4 gap-4 bg-slate-50 rounded-xl p-4">
    <div><div class="text-xs text-slate-500">Total Gabus Detail</div><div class="font-bold" id="sum_boxes">0</div></div>
...
const buyersOptions = `@foreach($buyers as $buyer)<option value="{{ $buyer->id }}">{{ addslashes($buyer->name) }}</option>@endforeach`;
...
function addItemRow(){
    ...
    tr.innerHTML = `<td class="p-2"><select name="items[${itemIndex}][buyer_id]" required class="rounded-lg border-slate-300 min-w-48"><option value="">Pilih</option>${buyersOptions}</select></td><td class="p-2"><input name="items[${itemIndex}][fish_type]" class="rounded-lg border-slate-300 min-w-32"></td><td class="p-2"><input type="number" min="1" name="items[${itemIndex}][box_count]" oninput="recalc()" required class="box-count rounded-lg border-slate-300 w-24 text-right"></td><td class="p-2"><input type="number" min="0" name="items[${itemIndex}][price_per_box]" oninput="recalc()" required class="price-per-box rounded-lg border-slate-300 w-32 text-right"></td><td class="p-2 text-right subtotal">Rp0</td><td class="p-2"><button type="button" onclick="removeItemRow(this)" class="text-red-600">Hapus</button></td>`;

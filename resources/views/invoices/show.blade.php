...
<div class="mb-5 flex flex-col md:flex-row md:items-start md:justify-between gap-3">
    <div><a href="{{ route('invoices.index') }}" class="inline-flex text-sm font-semibold text-slate-500 mb-3">← Daftar invoice</a><h1 class="text-2xl md:text-3xl font-black">{{ $invoice->invoice_number }}</h1><p class="text-sm text-slate-500 mt-1">{{ $invoice->invoice_date->format('d/m/Y') }} · {{ $invoice->ship?->name }} · {{ strtoupper($invoice->status) }}</p></div>
    <div class="grid grid-cols-2 md:flex gap-2">
        ...
        @if(in_array($invoice->status, ['draft','posted']))<form method="POST" action="{{ route('invoices.cancel', $invoice) }}" onsubmit="return confirm('Batalkan invoice ini?')">@csrf<button class="w-full rounded-2xl bg-rose-600 px-4 py-3 text-sm font-bold text-white">Cancel</button></form>@endif
        <a href="{{ route('invoices.print', $invoice) }}" target="_blank" class="rounded-2xl bg-teal-600 px-4 py-3 text-center text-sm font-bold text-white">Print</a>
    </div>
</div>
